 ...
 *         Time: 1:34:27 PM
 */
public final class LongDenseAbsoluteList implements LongDenseList {
    /**
    ...
    public LongDenseAbsoluteList(final byte[] offsetRawData, final int maxReads,
                                 final int numOffsets) throws IOException {
        this(new InputBitStream(offsetRawData), maxReads, numOffsets);
    }

    public LongDenseAbsoluteList(final InputBitStream offsetRawData,
                                 final int maxReads,
                                 final int numOffsets) throws IOException {
    ...
    private void prepareRandomAccess(final int numOffsets) throws IOException {
        ...
        for (int i = numOffsets; i > 0; --i) {
            final long offset = stream.readLongDelta();
