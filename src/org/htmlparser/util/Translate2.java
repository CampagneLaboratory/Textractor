...
public class Translate2
{
    ...
    /**
     * Decode a string containing references but maintains the
     * length of the string.
     * Change all numeric character reference and character entity references
     * to unicode characters.
     * @param string The string to translate.
     */
    public static String decodeSameLength(String string)
    {
            ...
            do
            {
                ...
                if (index < length)
                {
                    ...
                    if ('#' == character)
                    {
                        ...
                        int numProcessed = 0;  // keep track of the characters processed so we can pad later
                        while ((i < length) && !done)
                        {
                        ...
                        if (0 != number)
                        {
                            ...
                            index = i;
                            amp = index;
                            // pad the buffer with the characters that were processed to get the number
                            for (int count = 0; count < (numProcessed + 1); count++) {
                                buffer.append("\0");
                            }
                        }
                    }
                    else if (Character.isLetter (character)) // really can't start with a digit eh...
                    {
                        ...
                        if (null != item)
                        {
                            ...
                            int extraChars = item.getKernel ().length ();
                            index += extraChars;
                            for (int count = 0; count < extraChars; count++) {
                                buffer.append("\0");
                            }
