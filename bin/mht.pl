...
for $i($start..$end){
    ...
    if ($index==1){
	    system("ant -f build/build.xml -DbuildIndex=1 -DpropertyFilename=textractor.properties.ht$htDatabaseNumber mht") && die "can not run";
    }else{
        system("ant -f build/build.xml -DpropertyFilename=textractor.properties.ht$htDatabaseNumber mht") && die "can not run";
    }
